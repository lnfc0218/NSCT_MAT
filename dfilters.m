...
switch fname
	...
	t = [0, 1, 0; 1, 0, 1; 0, 1, 0] / 4;	% diamond kernel为什么是这种核函数？？
	h0 = ftrans2(h0, t);
	h1 = ftrans2(h1, t);
 ...
 case {'pkva-half4'}	% Filters from the ladder structure	
   ...
   case 'dmaxflat7'
       ...
       k3=k1;
       k2 = M1;          
       h  = [.25*k2*k3 .5*k2 1+.5*k2*k3]*M1; 
       h = [h fliplr(h(1:end-1))];
       g  = [-.125*k1*k2*k3 0.25*k1*k2 (-0.5*k1-0.5*k3-0.375*k1*k2*k3) 1+ .5*k1*k2]*M2;
       g = [g fliplr(g(1:end-1))];
       B  = dmaxflat(7,0);
       % produces the 2-D FIR filter that corresponds to
       %  the 1-D FIR filter B using the transform T.
       h0 = mctrans(h,B);
       g0 = mctrans(g,B);
       % energy normalization preprocessing
       h0 = sqrt(2)*(h0./sum(h0(:)));
       g0 = sqrt(2)*(g0./sum(g0(:)));
       h1 = modulate2(g0,'b' );
    if lower(type(1)) == 'r'
